...
public class KademliaRoutingTable
{
    ...
    {
        KademliaNode kn = new KademliaNode(node.getIp(),node.getId().toByteArray(),node.getPort(), node.getPublicKey().toByteArray());
        lock.lock();
        TreeNode curr = root;
        boolean ret ;
    ...
    private ArrayList<KademliaNode> findClosestNodeRec(TreeNode curr, byte[] nodeId, int i, int j,int a, String path)
    {
        ...
        if (curr != null)
        {
            ...
            if (curr.kc >= 1)
            {
                ...
                if (hasObject(curr.kbucket,new KademliaNode("",nodeId,421, publicKey.getEncoded())))
                {
